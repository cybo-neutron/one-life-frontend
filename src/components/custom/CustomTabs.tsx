  ...
  return (
    <Tabs defaultValue="notes" className="h-full w-full  ">
      <TabsList className="flex w-full justify-center items-center  bg-transparent gap-2 ">
        <div className="flex gap-0 bg-transparent border-[1px] border-secondary-100/20 rounded-sm  overflow-hidden  h-full">
          {tabItems.map((item, index) => (
            <>
              ...
              {index !== tabItems.length - 1 && (
                <Separator orientation="vertical" className="bg-secondary-100/20" />
              )}
            </>
          ))}
        </div>
      ...
      {tabItems.map((item, index) => (
        <TabsContent
          key={index}
          value={item.value}
          className="bg-secondary-800/20 h-full"
        >
